  ...
  for await (const dirent of dir) {
    fs.readFile(path.join(from, dirent.name), (err, data) => {
      if (err) {
        ...
        let d = post.date;
        let date_string = `${d.getFullYear()}-${d
          .getMonth()
          .toString()
        ...
        fs.writeFile(
          ...
          `${yaml.dump(post)}date: ${date_string}`,
          () => {
            console.log(
              `\x1b[32mFile \x1b[34m${path.join(
                from,
                dirent.name
              )}\x1b[32m processed.\x1b[0m`
            );
          }
        );
      }
...
ls("./assets/blog/entries", "./assets/blog/__processed_entries").catch(
  console.error
);
